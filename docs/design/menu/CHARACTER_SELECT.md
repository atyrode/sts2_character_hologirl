...
- Confirmed tester-preferred temporary framing values: X `821`, Y `94`, scale `1.53`, whip density `5.0`, drift density `5.0`, gold jitter `80`, hologram tint `1.0`, tear `0.0`, tear frequency `4.0`, shimmer `0.04`, scanline `0.34`, scan speed `0.43`, scan spacing `87.0`, body opacity `0.86`, background `Periwinkle Drift`, character `Thin Outline Compact`. These are now the tuner defaults until replaced by final constants.
...
- Runtime character select should use the stable PNG cutout variants. The tuner is temporary calibration tooling and should start hidden behind `F3`.
...
- The no-reference linework batch is archived at `docs/design/art_archive/menu/character_select_layers/hologirl_cutout/batch-2026-05-16-no-reference-linework/` and exposed in the F3 tuner as `Linework No Ref 01` through `Linework No Ref 10`. The tuner panel visibility is intentionally saved in script-level static state so enabling it persists while switching away from and back to Hologirl during character-select comparison.
- Hologirl's character-select sound is currently `Hologirl/audio/char_select/hologram.mp3`, played by the character-select background scene when it is instantiated. This is a pragmatic scene-level hook rather than a confirmed vanilla `CharacterModel` sound property.

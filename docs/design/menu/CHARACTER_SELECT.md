...
- `v0.2.37` adds `Puppet Attempt 002` as a selectable in-game tuner variant for direct comparison. It is intentionally experimental: the stable `Chunky Vanilla` cutout remains the default, while `Puppet Attempt 002` assembles the generated rig-sheet parts from `Hologirl/images/charui/puppet_attempt_002/` with very small per-part rotations.
- The puppet needs a pose-calibration pass before it can be considered for final character-select art. The temporary tuner now exposes a puppet section only when `Puppet Attempt 002` is selected: choose a part, adjust X/Y/rotation/scale, optionally disable motion, and use `Copy Pose` to export the current offsets. Those copied values are the intended handoff for hardcoding a corrected rest pose.

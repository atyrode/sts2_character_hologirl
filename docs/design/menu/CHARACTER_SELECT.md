...
- Current Hologirl character motion is an overlay illusion over a single foreground cutout. `v0.2.31`/`v0.2.32` shipped a broad blue/gold masked-overlay drift, and `v0.2.33` narrowed that into separate masked overlays for the whip, ponytail regions, and visible arm region. `v0.2.34` added actual transparent PNG layers for `Chunky Vanilla` under `Hologirl/images/charui/character_layers/`, but kept the full original cutout under them, which made the motion too easy to miss. The next local fix uses `chunky_base.png` as the stable base for `Chunky Vanilla`, then renders the moving whip, ponytail, and arm layers at full opacity with larger but still subtle offsets. This is still not true skeletal animation, but it is closer to a vanilla-style layered scene.

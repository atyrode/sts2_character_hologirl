...
- Tuner ranges are intentionally broad while calibrating: X `-500..2600`, Y `-700..1200`, scale `0.10..3.50`, particle densities `0..10`, gold jitter `0..160`, hologram tint/shimmer/scanline `0..1.5`, tear `0..0.030`, tear frequency `0.25..5.0`, scan speed `-5..5`, scan spacing `4..180`, and body opacity `0.25..1.25`.
- Confirmed tester-preferred default framing values: X `937`, Y `139`, scale `1.33`, whip density `1.53`, drift density `6.05`, gold jitter `80`, hologram tint `0.62`, tear `0.0`, tear frequency `0.7`, shimmer `0.72`, scanline `0.25`, scan speed `1.08`, scan spacing `87.0`, body opacity `0.83`, background `Periwinkle Drift`, character `Linework No Ref 02`.
- Tear frequency currently controls temporal cadence for short tearing pulses, not constant vertical band density. Higher values mean rarer tear pulses; the top of the current range is roughly one pulse opportunity every five seconds.
- Body opacity fades Hologirl's non-gold cutout pixels to help the character read as a hologram while mostly preserving the golden whip and outfit/tool accents.
...
- Runtime character select should ship one production background and one production character cutout. Variant explorations belong in `docs/design/art_archive/`, not under `Hologirl/`, and should not be wired into runtime selectors.
- The 2026-05-16 humanoid hologram cutout batch explores variants that keep the accepted white/gold outfit, golden whip, broad cel-shaded forms, and pastel grit while reducing the gem/slime read. Archive: `docs/design/art_archive/menu/character_select_layers/hologirl_cutout/batch-2026-05-16-humanoid-hologram/`.
- Runtime character-select art is exported at `1282x602` and stretched to the `2564x1204` virtual canvas. The archived originals remain design history, while `Hologirl/images/charui/character_select_bg.png` and `Hologirl/images/charui/character_select_hologirl.png` are the production runtime assets.
...
- As of `v0.2.48`, the runtime character-select button uses the `Linework No Ref 02` close-up review `variant-002` over `Periwinkle Drift`, archived under `docs/design/art_archive/menu/character_select_button/linework-no-ref-02-review/` and written to `Hologirl/images/charui/char_select_char_name.png`.
- The attempted character motion overlay is rejected. `v0.2.31`/`v0.2.32` shipped a broad blue/gold masked-overlay drift, `v0.2.33` narrowed that into separate masked overlays, and `v0.2.34`/`v0.2.35` tried generated transparent cutout layers for `Chunky Vanilla`. The result exposed bad seams, overlaps, and cutoffs because the source art was not authored as animation-ready layers. Runtime now uses a single stable character cutout plus particles only. Keep the archived layer attempts in `docs/design/art_archive/menu/character_select_layers/` as a record, but do not reintroduce layered body motion unless the character is redrawn as proper separated parts with intentional overlap margins and pivots.
- `v0.2.37`/`v0.2.38` tested `Puppet Attempt 002` as an in-game tuner variant. That path is rejected for now: the generated pieces are too disjointed and need real artist-authored assembly, z-ordering, pivots, and rest-pose work before they are worth runtime integration. Keep the archived puppet work as design research only.
- Runtime character select should use the single selected PNG cutout. The tuner is temporary calibration tooling and should start hidden behind `F3`.
- Next character cutout direction, based on the latest reviewed generated passes: keep the `Thin Outline Compact` / white-and-gold Hologirl design, close action pose, faceless two-eye hologram face, large blue twin-tail silhouette, and golden light-whip. Add vanilla-like black/dark-cyan linework only as thin, broken shadow and occlusion accents, not all-over comic inking or a continuous sticker outline. Hair should read as soft hand-painted hologram hair masses with broad painted shadow cuts, not faceted crystal or gemstone shading. Both forearms should share the same accessory grammar: exactly two gold circlets on each forearm with one uninterrupted white glove/cuff span between them; no third gold band and no split extra cuff on the left/weapon arm. Because reference-image editing repeatedly copied the incorrect left-arm accessory, the next exploration should generate no-reference variants from this written design spec.
- The no-reference linework batch is archived at `docs/design/art_archive/menu/character_select_layers/hologirl_cutout/batch-2026-05-16-no-reference-linework/`. `Linework No Ref 02` is the selected production cutout copied to `Hologirl/images/charui/character_select_hologirl.png`; the other generated attempts are archive-only.
- Hologirl's character-select sound is currently `Hologirl/audio/char_select/hologram.mp3`, played by the character-select background scene when it is instantiated. This remains a scene-level hook while Hologirl uses `CustomCharacterModel` and avoids placeholder character-select sounds.

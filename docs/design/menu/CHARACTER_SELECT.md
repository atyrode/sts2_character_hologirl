...
- Current Hologirl character motion is particle-only over a single foreground cutout. Vanilla character-select scenes appear to use packed scene layering and node animation to create subtle living motion. Hologirl can move toward that, but the compatible route is to author separate character layers and animate them inside `hologirl_character_select_bg.tscn`, not to stretch the whole PNG or patch vanilla UI nodes.

...
- Current Hologirl character motion is an overlay illusion over a single foreground cutout. `v0.2.31`/`v0.2.32` shipped a broad blue/gold masked-overlay drift; the next local pass narrows that into separate masked overlays for the whip, ponytail regions, and visible arm region. This can create very slight living motion, but it is still not true skeletal or layered character animation. The proper final route is to author separate transparent art layers for body, ponytails, arms, and whip, then animate those inside `hologirl_character_select_bg.tscn`.
